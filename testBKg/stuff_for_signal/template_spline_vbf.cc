...
#include "template_spline_vbf.h"


void template_spline_vbf(){

	
	for(int i=0; i<sizeof(ch_list)/sizeof(ch_list[0]); i++){
//		hist_template = extract_templates(hist_input);	
		TFile *f_spline = new TFile(Form(spline_input+"%s_spline_WS_vbf.root", (ch_list[i]).c_str()));
...
//		coeff_list[i] = extract_coeff(ws, ch_list[i]);
	 ...
	 cout<<temp_form<<endl;
         temp_form = (RooFormulaVar*) ws->obj(Form("qqH_f0_final%s", (ch_list[i]).c_str()));
	 cout<<temp_form<<endl;
         form_list.add(*temp_form);
         temp_form = (RooFormulaVar*) ws->obj(Form("qqH_f1_final%s", (ch_list[i]).c_str()));
         form_list.add(*temp_form);
         temp_form = (RooFormulaVar*) ws->obj(Form("qqH_f2_final%s", (ch_list[i]).c_str()));
         form_list.add(*temp_form);
	form_list.Print();
...
//		pdf_list[i] = prod_pdf(ws, hist_input, ch_list[i], i);	
        TFile f_hist(hist_input+ch_list[i]+"_vbf.root");
//        vector<TH2F*> th2fs(3);
...
TCanvas *c = new TCanvas("","");
	...
	dbkg.setBins(10);
		...
		c->SaveAs(Form("~/www/170627_check_th2f_r%d_before_datahist.png",j));
                ...
                datahist_temp = new RooDataHist(Form("datahist_r%d_%s_vbf",j,(ch_list[i]).c_str()),"",argset, th2fs[i][j]);    
                datahists.push_back(datahist_temp);
                histpdf_temp = new RooHistPdf("histpdf", "", argset, *datahists[j]);
		histpdf_temp->SetNameTitle(Form("r%d_template_%s_vbf",j,(ch_list[i]).c_str()),Form("r%d_template_%s_vbf",j,(ch_list[i]).c_str()));
...
//        RooArgList prods;
        ...
        if(sp_size==th2fs[i].size()){
                for(int j=0; j<sp_size; j++){
                        prod_temp = new RooProdPdf(Form("prodPdf_%s_r%d_vbf", (ch_list[i]).c_str(), j),Form("prodPdf_%s_r%d_vbf", (ch_list[i]).c_str(), j), *(spline_template[i][j]), Conditional(*(histpdfs[j]),dbkg));
...
//**************** Ending pdf_list collection **********************
		...
		pdf_list[i].Print();
		coeff_list[i].Print();
		RooRealSumPdf* ggH = new RooRealSumPdf("qqH", "qqH", pdf_list[i], coeff_list[i]);	
	        RooFormulaVar *ggH_norm = new RooFormulaVar("qqH_norm","qqH_norm","@0+@1+@2",coeff_list[i]);
		
//	return;
		TFile *f= new TFile(Form("%s_2d_spline_WS_vbf.root",(ch_list[i]).c_str()), "recreate");
		f->cd();
...
RooArgList extract_coeff(RooWorkspace* w, string tag){
	 RooArgList form_list;
	 RooFormulaVar* temp_form;
	 temp_form = (RooFormulaVar*) w->obj(Form("qqH_f0_final%s", tag.c_str()));
	 form_list.add(*temp_form);
	 temp_form = (RooFormulaVar*) w->obj(Form("qqH_f1_final%s", tag.c_str()));
	 form_list.add(*temp_form);
	 temp_form = (RooFormulaVar*) w->obj(Form("qqH_f2_final%s", tag.c_str()));
	 form_list.add(*temp_form);
...
vector<RooNCSplinePdf_3D_fast*> extract_splines(RooWorkspace* w, string channel){
	...
	sp_temp = (RooNCSplinePdf_3D_fast*) w->pdf("qqH_r0");
	splines.push_back(sp_temp);
	sp_temp = (RooNCSplinePdf_3D_fast*) w->pdf("qqH_r1");
	splines.push_back(sp_temp);
	sp_temp = (RooNCSplinePdf_3D_fast*) w->pdf("qqH_r2");
	splines.push_back(sp_temp);
	for(int i=0; i<splines.size(); i++){
		cout<<splines[i]<<endl;
		splines[i]->SetNameTitle(Form("spline_%s_r%d_vbf", channel.c_str(), i),Form("spline_%s_r%d", channel.c_str(), i));
	}
